...
Return your analysis as structured JSON matching the output schema."""
    ...
    def run(
        ...
        print(f"\n[DEBUG FORETHOUGHT] Vector search returned {len(search_results)} results")
        for i, result in enumerate(search_results[:3], 1):
            print(f"[DEBUG] Result {i}:")
            print(f"  Guest: {result.get('guest', 'N/A')}")
            print(f"  Episode: {result.get('episode_id', 'N/A')}")
            print(f"  Similarity: {result.get('similarity', 0):.3f}")
            print(f"  Has contrarian signal: {result.get('has_contrarian_signal', False)}")
        ...
        print(f"[DEBUG FORETHOUGHT] Formatted search context length: {len(search_context)} chars")
        print(f"[DEBUG FORETHOUGHT] First 500 chars of prompt:\n{search_context[:500]}")
        print()
...
Analyze these search results and identify contrarian perspectives.
Return structured JSON with your findings.

Remember:
- Include results that DISAGREE with, CHALLENGE, ADD IMPORTANT NUANCE to, or provide ALTERNATIVE PERSPECTIVES on the conventional wisdom
- Even partial disagreements, important caveats, or situational exceptions are valuable
- Extract the strongest quote that captures their perspective
- Rate relevance 1-10 based on how useful their perspective is for understanding the full picture"""
